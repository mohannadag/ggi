...
this.L.Control.Geocoder = (function(L) {
    ...
    var LatLng = L.Class.extend({
        options: { next: undefined, sizeInMeters: 10000 },
        initialize: function(options) {
    ...
    var Nominatim = L.Class.extend({
        ...
        geocode: function(query, cb, context) {
            getJSON(
                ...
                L.extend(
                    { q: query, limit: 5, format: "json", addressdetails: 1 },
    ...
    var OpenLocationCode = L.Class.extend({
        options: { OpenLocationCode: undefined, codeLength: undefined },
        initialize: function(options) {
            L.setOptions(this, options);
        },
        geocode: function(query, cb, context) {
    ...
    var Pelias = L.Class.extend({
        ...
        geocode: function(query, cb, context) {
            ...
            getJSON(
                this.options.serviceUrl + "/search",
                L.extend(
                    { api_key: this._apiKey, text: query },
                    this.options.geocodingQueryParams
                ),
                function(data) {
    ...
    var Openrouteservice = Mapzen.extend({
        options: { serviceUrl: "https://api.openrouteservice.org/geocode" }
    });
    ...
    var What3Words = L.Class.extend({
        ...
        geocode: function(query, cb, context) {
            getJSON(
                ...
                { key: this._accessToken, addr: query.split(/\s+/).join(".") },
                function(data) {
                    var results = [],
                        latLng,
                        latLngBounds;
    ...
    L.Util.extend(L.Control, { Geocoder: Geocoder, geocoder: geocoder });
    return Geocoder;
})(L);
