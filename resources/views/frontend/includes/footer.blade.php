            ...
            <div class="col-span">
                <div class="flex flex-wrap items-center justify-between">
                    <div class="w-full lg:w-auto">
                        <h3 dir="{{(App::isLocale('ar') ? 'rtl' : 'ltr')}}" class="font-lora text-white text-[24px] sm:text-[30px] xl:text-[36px] leading-[1.277] mb-[10px]">{{trans('file.subscribe_newsletter')}}</h3>
                        <p class="text-secondary leading-[1.5] tracking-[0.03em] mb-10">{{trans('file.newsletter_content')}}</p>
                        <form id="mc-form" class="relative w-full">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <input id="mc-email" class="font-light text-white leading-[1.75] opacity-100 border border-secondary w-full lg:w-[395px] xl:w-[495px] h-[60px] rounded-[10px] py-[15px] pl-[15px] pr-[15px] sm:pr-[135px] focus:border-white focus:outline-none border-opacity-60 placeholder:text-[#E2E2E2] bg-transparent" type="text" placeholder="{{trans('file.email')}}...">
                            <button id="mc-submit" type="submit" class="text-white font-medium text-[16px] leading-none tracking-[0.02em] bg-secondary py-[17px] px-[20px] mt-5 sm:mt-0 rounded-[10px] hover:bg-white hover:text-primary transition-all sm:absolute sm:right-[4px] sm:top-1/2 sm:-translate-y-1/2">{{trans('file.subscribe')}}</button>
                        </form>
                    ...
                    <div class="w-full hidden lg:block lg:w-auto mt-5 lg:mt-0">
                        <div class="relative mt-10 md:mt-0 lg:absolute lg:{{ App::isLocale('ar') ? 'left' : 'right' }}-0 lg:bottom-0">
                            <img class="hero_image lg:max-w-[550px] xl:max-w-[650px] 2xl:max-w-[714px]" src="{{asset('frontend/images/newsletter/bg-1.png')}}" width="866" height="879" style="{{ App::isLocale('ar') ? '-webkit-transform; scaleX(-1); transform: scaleX(-1);' : '' }}" alt="hero image">
                        </div>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-12 gap-x-[30px] mb-[-30px]">
            <div class="col-span-12 sm:col-span-6 lg:col-span-4 mb-[30px]">
                <a href="{{ url('/') }}" class="block mb-[25px]">
                    <img src="{{asset('frontend/images/logo/logo.png')}}" width="99" height="46" loading=lazy alt="Golden Group Investment Logo">
                </a>
                <p class="mb-[5px] xl:mb-[40px] max-w-[270px]">{{trans('file.footer_desc')}}</p>
                <p class="text-sm hidden md:block"><a href="{{url('/')}}" class="text-secondary"> {{trans('file.copyright')}}</a></p>
            </div>
            <div class="col-span-12 sm:col-span-6 lg:col-span-3 mb-[30px]">
                <h3 class="font-lora font-normal text-[22px] leading-[1.222] text-primary mb-[20px] lg:mb-[30px]">
                    {{trans('file.about_footer')}}<span class="text-secondary">.</span></h3>
                <ul class="text-[16px] leading-none mb-[-20px]">
                    <li class="mb-[20px]"><a class="inline-block transition-all hover:text-secondary" href="{{url('/about')}}">{{trans('file.about')}}</a></li>
                    <li class="mb-[20px]"><a class="inline-block transition-all hover:text-secondary" href="{{url('/properties')}}">{{trans('file.turkey_properties')}}</a></li>
                    <li class="mb-[20px]"><a class="inline-block transition-all hover:text-secondary" href="{{url('/career')}}">{{trans('file.careers')}}</a></li>
                    <li class="mb-[20px]"><a class="inline-block transition-all hover:text-secondary" href="{{url('/news')}}">{{trans('file.news')}}</a></li>
                </ul>
            </div>
            <div class="col-span-12 sm:col-span-6 lg:col-span-3 mb-[30px]">
                <h3 class="font-lora font-normal text-[22px] leading-[1.222] text-primary mb-[20px] lg:mb-[30px]">
                    {{trans('file.ggi_listings')}}<span class="text-secondary">.</span></h3>
                <ul class="text-[16px] leading-none mb-[-20px]">
                    @foreach($states->where('status',1) as $state)
                    <li class="mb-[20px]"><a class="inline-block transition-all hover:text-secondary" href="{{route('property.state',$state)}}">{{ $state->stateTranslation->name ?? ($state->stateTranslationEnglish->name ?? null) }}</a></li>
                    @endforeach
                </ul>
            </div>
            <div class="col-span-12 sm:col-span-6 lg:col-span-2 mb-[30px]">
                <h3 class="font-lora font-normal text-[22px] leading-[1.222] text-primary mb-[20px] lg:mb-[30px]">
                    {{trans('file.our_address')}}<span class="text-secondary">.</span></h3>
                <p>{{isset($siteInfo->address) ? $siteInfo->address : 'address'}}</p>
                <ul class="inline-flex items-center justify-center">
                    <li class="">
                        <a href="{{ App::isLocale('ar') ? 'https://www.facebook.com/GGITurkey' : 'https://www.facebook.com/profile.php?id=100063781434306' }}" aria-label="svg" class="w-[22px] h-[22px] transition-all rounded-full bg-primary flex items-center justify-center hover:drop-shadow-[0px_4px_10px_rgba(0,0,0,0.25)] text-white hover:bg-secondary">
                    ...
                    <li class="{{ App::isLocale('ar') ? 'mr-[15px]' : 'ml-[15px]' }}">
                        <a href="{{isset($siteInfo->twitter) ? $siteInfo->twitter : '#'}}" aria-label="svg" class="w-[22px] h-[22px] transition-all rounded-full bg-primary flex items-center justify-center hover:drop-shadow-[0px_4px_10px_rgba(0,0,0,0.25)] text-white hover:bg-secondary">
                    ...
                    <li class="{{ App::isLocale('ar') ? 'mr-[15px]' : 'ml-[15px]' }}">
                        <a href="{{ App::isLocale('ar') ? 'https://www.instagram.com/ggiturkey' : 'https://www.instagram.com/ggiturkey.global' }}" aria-label="svg" class="w-[22px] h-[22px] transition-all rounded-full bg-primary flex items-center justify-center hover:drop-shadow-[0px_4px_10px_rgba(0,0,0,0.25)] text-white hover:bg-secondary">
                ...
                <ul class="inline-flex items-center justify-center">
                    <li class="{{ App::isLocale('ar') ? 'ml-[15px]' : '' }}">
                        <a href="{{url('https://www.linkedin.com/company/golden-groupltd')}}" aria-label="svg" class="w-[22px] h-[22px] transition-all rounded-full bg-primary flex items-center justify-center hover:drop-shadow-[0px_4px_10px_rgba(0,0,0,0.25)] text-white hover:bg-secondary">
                    ...
                    <li class="ml-[15px]">
                        <a href="https://www.tiktok.com/@ggiturkey" aria-label="svg" class="w-[22px] h-[22px] transition-all rounded-full bg-primary flex items-center justify-center hover:drop-shadow-[0px_4px_10px_rgba(0,0,0,0.25)] text-white hover:bg-secondary">
                            <img width="10px" src="{{asset('/images/tiktok.png')}}">
                        </a>
                    </li>
                    <li class="ml-[15px]">
                        <a href="https://www.youtube.com/@goldengroupinvestment" aria-label="svg" class="w-[22px] h-[22px] transition-all rounded-full bg-primary flex items-center justify-center hover:drop-shadow-[0px_4px_10px_rgba(0,0,0,0.25)] text-white hover:bg-secondary">
                            <img width="10px" src="{{asset('/images/youtube.png')}}">
                        </a>
                    </li>
                </ul>
            </div>
